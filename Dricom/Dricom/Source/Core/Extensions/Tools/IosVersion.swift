...
public struct IosVersion: Comparable {
    ...
    init(_ versionString: String) {
        version = versionString.components(separatedBy: ".").map{ $0.toInt() }.flatMap{ $0 }
    }
}
...
extension UIDevice {
    ...
    // Convenience
    public class var iosVersion: IosVersion {
        return current.iosVersion
    }
}
...
public func ~= <T: Comparable>(pattern: ClosedRange<T>, value: IosVersion) -> Bool {
    return pattern.lowerBound <= value && value <= pattern.upperBound
}
public func ~= <T: Comparable>(pattern: Range<T>, value: IosVersion) -> Bool {
    return pattern.lowerBound <= value && value < pattern.upperBound
}
