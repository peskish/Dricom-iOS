...
@IBDesignable open class HoshiTextField: TextFieldEffects {
    ...
    */
    @IBInspectable dynamic open var placeholderEditingFontScale: CGFloat = 0.65 {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    /**
     ...
     This property determines the size of the placeholder label relative to the font size of the text field.
     */
    @IBInspectable dynamic open var placeholderNormalFontScale: CGFloat = 0.65 {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    override open func drawViewsForRect(_ rect: CGRect) {
        ...
        if isFirstResponder {
            placeholderLabel.font = placeholderEditingFontFromFont(font!)
        } else {
            placeholderLabel.font = placeholderNormalFontFromFont(font!)
        }
    ...
    override open func animateViewsForTextDisplay() {
        if text!.isEmpty {
            UIView.animate(withDuration: 0.35, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 2.0, options: UIViewAnimationOptions.beginFromCurrentState, animations: ({
                if self.placeholderNormalFontScale != self.placeholderEditingFontScale {
                    self.placeholderLabel.font = self.placeholderNormalFontFromFont(self.font!)
                }
    ...
    private func placeholderEditingFontFromFont(_ font: UIFont) -> UIFont! {
        let smallerFont = UIFont(name: font.fontName, size: font.pointSize * placeholderEditingFontScale)
        return smallerFont
    }
    
    private func placeholderNormalFontFromFont(_ font: UIFont) -> UIFont! {
        let smallerFont = UIFont(name: font.fontName, size: font.pointSize * placeholderNormalFontScale)
        return smallerFont
    }
